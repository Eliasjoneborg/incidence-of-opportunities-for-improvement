...
#JA: Överväg att byta 151:201 till ett vilkor för AIS, se tidigare kod för blunt multisystem
...
# JA:Definition:  penetrating truncal injury with an AIS severity score of > 3 in the regions of the neck, chest, or abdomen? Så AIS ska ha första siffran 3-5. och i någon av dem ska vara AIS, alltså sista siffran vara >= 3.+ penetrerande som huvudskada. 

#Vektor för att testa: 
vector_of_codes <- c(161006.3, 541432.4, 377120.1) # Tipps, Skapa En fiktiv vektor med AIS-koder, byt manuellt för att testa dina funktioner. 
neck_chest_abdomen_region(vector_of_codes)
substr(as.character(vector_of_codes), 1, 1) # genererar en vector med första elementen, du vill ju veta om någon av dem är 3-5.

is.element(substr(as.character(vector_of_codes), 1, 1),3:5) # True/false för respeketive element däremot!

#neck_chest_abdomen_region <- function(code) {
 # code <- code[!is.na(code)]                     #Tidigare kod
  #region <- substr(as.character(code), 1, 1)
  #as.numeric(region) >= 3 && as.numeric(region)<=5
#}

#nedan genererar vektor True/false om det är korrekt region. 
neck_chest_abdomen_region <- function(code) {
  code <- code[!is.na(code)]
  region <- substr(as.character(code), 1, 1)  
...
#neck_chest_abdomen_region(code[serious_injury]) genererar en vektor True/false, (hoppar över alla skador som inte är AIS 3+) och säger om det är i rätt region. sum(neck_chest_abdomen_region(code[serious_injury]) == TRUE ) summerar alla gånger den vektorn är TRUE.
...
vector_of_codes <- c(361006.1, 141432.5, 577120.3)#Testa och ändra AISkoderna och se hur funktionen byter output. 
serious_neck_chest_abdomen_injury(vector_of_codes)

#Ändrat nedan, nu vill vi att serious_neck_chest_abdomen_injury() är >= 1.
