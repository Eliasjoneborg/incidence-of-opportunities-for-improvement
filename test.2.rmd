...
[8]: paste0('Shock (n = ', a$i, ')')
[9]: paste0('Geriatric (n = ', a$h, ')')", height = "500") %>% DiagrammeRsvg::export_svg() %>% charToRaw() %>% rsvg::rsvg_pdf("exclusion_flowchart.pdf")   
...
#Yearly incidence as %
plot_inc_year <- incidence.data %>% select(dates, incidence_blu, incidence_pen, incidence_blu, incidence_sho, incidence_ger, incidence_all)
...
## Participants 
Over the pre-specified study period, `r nrow(datasets$problem)` patients were included in the the KUH trauma care quality database. Of those, `r nrow(fewer_variables)` (`r round(nrow(fewer_variables)/nrow(datasets$problem)*100, digits=1)`%) patients were aged 14 or above and had sufficient data regarding OFI (see figure \@ref(fig:exclusion-flowchart)). The blunt multisystem cohort was comprised of `r nrow(blunt_multisystem_cohort)` patients; penetrating of `r nrow(penetrating_cohort)` patients; severe TBI of `r nrow(severe_tbi_cohort)` patients; shock of `r  nrow(shock_cohort)` patients and geriatric of `r nrow(geriatric_cohort)` patients. Four categories of data were necessary for grouping: emergency department gcs and systolic blood pressure, dominant injury type (blunt or penetrating) and age. `r Exclusion_table$Missing[3]` patients lacked data about emergency department gcs and `r Exclusion_table$Missing[4]` patients lacked data about emergency department blood pressure. No patients lacked data about dominant injury type  or age (see Table \@ref(tab:exclusion-table)).

```{r exclusion-flowchart, echo=FALSE, fig.cap="\\textbf{Exclusion flowchart.} Excluded are all patients patients treated between 2013 and 2017, as they systematically lack data about OFI. Furthermore, excluded are patients aged 14 and below. \\emph{TBI: traumatic brain injury.}", out.width="100%", fig.align="center"}
...
```{r descriptive-table, echo=FALSE, warning=FALSE}
library(dplyr)
library(kableExtra)
...
## Incidence
In the study population, total incidence of OFI ranged from  `r round(min(incidence.data$incidence_all), digits=2)`% to   `r round(max(incidence.data$incidence_all), digits=2)`% (see figure \@ref(fig:incidence-graph)). The highest recorded biannual incidence was `r round(max(incidence), digits=2)`% in the `r colnames(incidence)[apply(cum_inc,1,which.max)]` cohort. The cumulative incidence rate varied across cohorts, as well as temporally within cohorts (see figure \@ref(fig:cumulative-incidence-graph)). The cohort with the largest cumulative incidence was `r colnames(cum_inc)[apply(cum_inc,1,which.max)]` (`r round(max(cum_inc), digits=2)`%)  and the cohort with the lowest was `r colnames(cum_inc)[apply(cum_inc,1,which.min)]` (`r round(min(cum_inc), digits=2)`%). The cumulative incidence in the trauma population in its entirety was `r round(max(cum_inc$cum_inc_all), digits=2)`%.

```{r incidence-graph, fig.cap="\\textbf{Biannual incidence of OFI.} Percentage of OFI of cases reviewed with a six month interval (2017-2021). \\emph{OFI: opportunities for improvement, TBI: traumatic brain injury.}", latex_options = "HOLD_position", echo=FALSE, warning=FALSE, out.width="85%", fig.align="center"}
library(ggplot2)
incidence_plot
```
...
## Trends
Table \@ref(tab:trend-table) presents the outcome of a fitted linear model with a time series component. All cohorts had a negative incidence trend estimate. With statistical significance defined as p<0,05, no trends were considered significant. The steepest trend estimate were that of Severe TBI with -0.017 (CI y-y, p=0,089). Closest to having a statistically significant trend was the shock cohort with p=0,0517.
...
incidence_trend_table %>%
  kbl(caption = "\\textbf{Incidence trends}", col.names = c("Cohort","Estimate",
                           "Standard error",
                           "Statistic", "P value")) %>%
