...
swetrau.20210602.20230228$arrival <- as.POSIXct(strptime(swetrau.20210602.20230228$DateTime_ArrivalAtHospital, format = "%Y-%m-%d %H:%M"))
fmp$arrival <- as.POSIXct(strptime(fmp$Ankomst_te, format = "%Y%m%d %H:%M"))
fmp.20210602.20230228$arrival <- as.POSIXct(strptime(fmp.20210602.20230228$Ankomst_te, format = "%Y%m%d %H:%M"))
...
swetrau.20210602.20230228$id <- paste(swetrau.20210602.20230228$arrival, swetrau.20210602.20230228$PersonIdentity, swetrau.20210602.20230228$TempIdentity)
fmp$id <- paste(fmp$arrival, fmp$Personnummer, fmp$Reservnummer)
fmp.20210602.20230228$id <- paste(fmp.20210602.20230228$arrival, fmp.20210602.20230228$Personnummer, fmp.20210602.20230228$Reservnummer)
problem$id <- paste(problem$arrival, problem$Personnummer, problem$Reservnummer)

## Combine relevant datasets

swetrau.combined <- rbind(swetrau, swetrau.20210602.20230228)
problem.combined <- merge(fmp.20210602.20230228, problem, by= "id", all.x = TRUE, all.y=TRUE)

## Coalesce important columns turned .x and .y

problem.combined <- problem.combined %>% mutate(Ankomst_te = coalesce(Ankomst_te.x, Ankomst_te.y))
problem.combined <- problem.combined %>% mutate(Problemomrade_.FMP = coalesce(Problemomrade_.FMP.x, Problemomrade_.FMP.y))

## Final merge

combined.swetrau.problem <- merge(swetrau.combined, problem.combined, by = "id", all.x = TRUE)

## Create OFI

combined.swetrau.problem$OFI <- create_ofi(combined.swetrau.problem) 
combined.swetrau.problem$OFI <- with(combined.swetrau.problem, ifelse(is.na(combined.swetrau.problem$OFI), "No", combined.swetrau.problem$OFI))


## Filter out patients 2017 -

combined.datasets <- filter(combined.swetrau.problem, as.Date(strptime(combined.swetrau.problem$Ankomst_te, format = "%Y%m%d %H:%M")) >="2017-01-01")

## Creating a separate df for OFI=YES

known_problem_area <-filter(combined.datasets, OFI == "Yes")

## Exclude <14 years

age_over_14 <- filter(combined.datasets, pt_age_yrs>14)

## Drop NA in age column

age_over_14 %>% drop_na(pt_age_yrs)

## Remove unneccessary variables

fewer_variables<-combined.datasets %>% select(id, inj_dominant, Problemomrade_.FMP, res_survival, pt_age_yrs, Gender, OFI, NISS, ed_gcs_sum, pre_intub_type, pre_sbp_value, pre_sbp_rtscat, ed_sbp_value, ed_sbp_rtscat, Ankomst_te.x, starts_with("AISCode"))

## Change 999 to NA for grouping into cohorts
...
#returns true if AIS severity is 3 or greater
...
#should return true if "is_serious" and "head_region" returns true
...
#acts on vectors in "serious_injury", chooses only those which have region == 1
